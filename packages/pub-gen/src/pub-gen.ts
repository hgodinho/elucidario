...
inquirer.prompt(pubGenPrompt(args)).then(async (answers) => {
    const pubPath = path.resolve(args.path, "..", "..", "publications");
    ...
    try {
        ...
        try {
            const packageJson = createPackageJson(packageName, answers);
            ...
            if (answers.multiIdioma) {
                ...
                demoContent.forEach((file: string) => {
                    fs.copyFileSync(
                        path.resolve(args.path, "template", "content", file),
                        path.resolve(
                            dirName,
                            "content",
            ...
            switch (answers.preset) {
                ...
                case "apa":
                    demoRefs = fs.readdirSync(
                        path.resolve(args.path, "template", "references", "apa")
                    );
                    break;
            }
...
const createPackageJson = (name: string, answers: any) => {
    const toGdocVersion = JSON.parse(
        fs
            .readFileSync(
                path.resolve(args.path, "..", "md-to-gdoc", "package.json")
            )
            .toString()
    ).version;
